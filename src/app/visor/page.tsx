...
/* =========================================================================
   COMPONENTE: MODAL DE INFORME COMPLETO
   ========================================================================= */
...
/* =========================================================================
   PÁGINA PRINCIPAL: VISOR TÁCTICO
   ========================================================================= */
export default function VisorTacticoPage() {
  ...
  useEffect(() => {
    ...
    const map = new mapboxgl.Map({
      ...
      style: 'mapbox://styles/mapbox/dark-v11', // Tema oscuro táctico
      center: [-75.8281, 6.5562], // Centro en Occidente Antioqueño
      zoom: 10,
      pitch: 45, // Inclinación perspectiva 3D
      bearing: -10,
    });
    ...
    // Capturar coordenadas dinámicas del cursor sobre el mapa
    map.on('mousemove', (e) => {
      setCoordsCursor({
  ...
  // 3. Renderizado y filtrado de marcadores en el mapa
  useEffect(() => {
    ...
    alertasFiltradas.forEach((alerta) => {
      ...
      // Elemento HTML personalizado para el pin táctico
  ...
  // Handler para seleccionar alerta desde el sidebar
  ...
  return (
    <div className="w-full min-h-screen bg-[#0A0A0B] text-[#EDEAE3] font-sans flex flex-col overflow-hidden">
      
      {/* 1. BARRA SUPERIOR TÁCTICA */}
      <header className="w-full border-b border-[rgba(237,234,227,0.11)] bg-[#070708] px-4 py-2.5 flex flex-wrap items-center justify-between gap-4 z-20">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 font-mono text-xs tracking-wider text-[#FF9E1B]">
            ...
            <span className="font-bold">GEOSYS // VISOR TÁCTICO v4.2</span>
          </div>
          <span className="text-[rgba(237,234,227,0.2)]">|</span>
          <span className="font-mono text-[11px] text-[#9A968C] tracking-widest hidden sm:inline">
            COBERTURA: OCCIDENTE ANTIOQUEÑO
          </span>
        </div>

        {/* SELECTOR DE CAPAS */}
        <div className="flex items-center gap-1 bg-[#0A0A0B] p-1 border border-[rgba(237,234,227,0.11)]">
          {['TODAS', 'COMERCIO', 'VIAS', 'AGRO', 'HIDRO'].map((capa) => (
            <button
              key={capa}
              onClick={() => setCapaActiva(capa)}
              className={`px-3 py-1 font-mono text-[10px] tracking-wider transition-all uppercase ${
                capaActiva === capa
        ...
        {/* TELEMETRÍA Y COORDENADAS */}
        <div className="flex items-center gap-4 font-mono text-[11px] text-[#9A968C]">
          <div className="hidden md:flex items-center gap-2 border-r border-[rgba(237,234,227,0.11)] pr-4">
      ...
      {/* 2. ÁREA PRINCIPAL DE TRABAJO (MAPA + PANEL) */}
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 relative min-h-[calc(100vh-53px)]">
        
        {/* CONTENEDOR DEL MAPA (8 COLUMNAS) */}
        <div className="lg:col-span-8 bg-[#050506] relative flex flex-col justify-between p-4 overflow-hidden border-b lg:border-b-0 lg:border-r border-[rgba(237,234,227,0.11)] select-none">
          ...
          <div className="relative z-10 flex items-center justify-between gap-2 pointer-events-auto">
            <div className="bg-[#0B0B0D]/90 border border-[rgba(237,234,227,0.11)] px-3 py-1.5 font-mono text-[11px] tracking-wider text-[#EDEAE3] flex items-center gap-2 backdrop-blur-sm">
              <span className="w-2 h-2 rounded-full bg-[#55C97C]" />
              <span>13 MUNICIPIOS MONITOREADOS EN TIEMPO REAL</span>
            </div>
            ...
            <div className="flex gap-2">
              <button 
                ...
                className="p-2 bg-[#0B0B0D]/90 border border-[rgba(237,234,227,0.11)] text-[#9A968C] hover:text-[#EDEAE3] backdrop-blur-sm transition-all"
          ...
          <div className="relative z-10 flex flex-wrap items-center justify-between gap-4 bg-[#0B0B0D]/90 border border-[rgba(237,234,227,0.11)] p-3 backdrop-blur-sm pointer-events-auto">
            <div className="flex items-center gap-4 font-mono text-[10px] text-[#9A968C]">
              <span className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 rounded-full bg-[#FF9E1B]" /> COMERCIO
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 rounded-full bg-[#3B82F6]" /> VÍAS
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 rounded-full bg-[#22C55E]" /> AGRO
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 rounded-full bg-[#06B6D4]" /> HIDRÁULICO
              </span>
            </div>

            <div className="font-mono text-[10px] text-[#7A776E]">
              CURSOR: {coordsCursor.lat} {coordsCursor.lng}
            </div>
          </div>
        </div>

        {/* SIDEBAR DERECHO DE INFORMACIÓN Y ALERTAS (4 COLUMNAS) */}
        <div className="lg:col-span-4 bg-[#0B0B0D] flex flex-col h-full border-t lg:border-t-0 border-[rgba(237,234,227,0.11)] z-10">
          
          {/* HEADER DEL PANEL */}
          <div className="p-4 border-b border-[rgba(237,234,227,0.11)] flex items-center justify-between">
          ...
          {/* LISTA DE ALERTAS */}
          <div className="flex-1 overflow-y-auto divide-y divide-[rgba(237,234,227,0.08)] max-h-[50vh] lg:max-h-none">
            {alertasFiltradas.map((alerta) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="font-sans text-xs text-[#9A968C] font-semibold mb-2">
                    {alerta.titulo}
                  </p>

                  <p className="font-mono text-[11px] text-[#7A776E] line-clamp-2 leading-relaxed">
                    {alerta.descripcion}
                  </p>
                  ...
                  <div className="mt-2 pt-2 border-t border-[rgba(237,234,227,0.05)] flex items-center justify-between font-mono text-[10px] text-[#5E5B52]">
                    ...
                    {/* Al hacer clic en la flecha de la tarjeta se abre el informe */}
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setAlertaSeleccionada(alerta);
          ...
          {/* FICHA DETALLADA DEL EVENTO SELECCIONADO */}
          {alertaSeleccionada && (
            <div className="p-4 border-t border-[rgba(237,234,227,0.11)] bg-[#070708]">
              <div className="font-mono text-[10px] text-[#FF9E1B] tracking-widest uppercase mb-2 flex items-center justify-between">
                <span>FICHA DE MONITOREO TÁCTICO</span>
                <span>ID: {alertaSeleccionada.id}</span>
              </div>

              <div className="grid grid-cols-2 gap-2 mb-3">
                <div className="p-2 bg-[#0B0B0D] border border-[rgba(237,234,227,0.08)]">
                  <span className="block font-mono text-[9px] text-[#7A776E] uppercase">Módulo</span>
                  <span className="font-mono text-xs font-bold text-[#EDEAE3]">{alertaSeleccionada.modulo}</span>
                </div>
                <div className="p-2 bg-[#0B0B0D] border border-[rgba(237,234,227,0.08)]">
                  <span className="block font-mono text-[9px] text-[#7A776E] uppercase">Fiabilidad</span>
                  <span className="font-mono text-xs font-bold text-[#55C97C]">94.8% (IA)</span>
                </div>
              </div>
